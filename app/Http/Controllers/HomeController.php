...
class HomeController extends Controller
{
    public function index(Request $request)
    {
        ...
        // 🗺️ CARTOGRAPHIE DES DOSSIERS : On récupère les vrais noms depuis la BDD (remplie par l'API)
        $groupNames = Group::pluck('name', 'key')->toArray();
        $groupNames['retd'] = 'R&D'; // On ajoute manuellement le cas de RETD
        // --------------------------------------------------------
        ...
        // 📁 FILTRAGE INTÉRIEUR D'UN DOSSIER (Avec repli automatique sur retd)
        if ($tab === 'all' && !empty($selectedFolder) && $selectedFolder !== 'ALL_DOCS') {
            $allTagsDocs = $allTagsDocs->filter(function($doc) use ($selectedFolder, $groupNames) {
                $key = $doc->group_key ?: 'retd'; 
                $docGroupRealName = $groupNames[$key] ?? 'R&D';
                return strtoupper(trim($docGroupRealName)) === strtoupper(trim($selectedFolder));
            });
        }
        ...
        if ($tab === 'all' && $isRetd) {
            ...
            if (!empty($selectedFolder)) {
                ...
                if ($selectedFolder === 'ALL_DOCS') {
                    ...
                    $folderDocs = $allDocs->filter(function($doc) use ($selectedFolder, $groupNames) {
                        $key = $doc->group_key ?: 'retd'; 
                        $docGroupRealName = $groupNames[$key] ?? 'R&D';
                        return strtoupper(trim($docGroupRealName)) === strtoupper(trim($selectedFolder));
                    })->values();
                    ...
                    $documentsToDisplay = new \Illuminate\Pagination\LengthAwarePaginator(
                        $items, $folderDocs->count(), $perPage, $page,
                        ['path' => \Illuminate\Pagination\Paginator::resolveCurrentPath(), 'query' => $request->query()]
                    );
                }

            } else {
                // 📁 VUE RACINE : On regroupe par dossier (et les 'sans groupe' vont dans RETD)
                $groupedDocs = $query->get()->groupBy(function($doc) use ($groupNames) {
                    $key = $doc->group_key ?: 'retd'; 
                    $docGroupRealName = $groupNames[$key] ?? 'R&D';
                    return strtoupper($docGroupRealName);
                });
