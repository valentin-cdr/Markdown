...
@section('content')
@php
    // 🚀 CORRECTION 1 : Logique stricte des rôles Keycloak
    ...
    foreach ($sessionGroups as $g) {
        $gLower = strtolower($g);
        if (str_contains($gLower, 'glossaire')) {
    ...
    // Si l'utilisateur a les deux rôles, ses droits de créateur écrasent ceux de lecteur sur l'interface
    if ($isCreator) {
        $isLecteur = false;
    }
    
    $isAdmin = in_array('retd', $sessionGroups);
    
    // Le bouton "+ Nouveau" et l'onglet "Mes documents" ne sont visibles que pour les créateurs ou admins
    $showMyDocuments = $isCreator || $isAdmin;

    // Récupération du nom du groupe
    // Récupération du nom du groupe (Compatible avec le switcher de l'Admin)
    $groupName = null;
    if (auth()->check()) {
        $forcedKey = session('active_group_key');
        
        // Si on est Admin ET qu'on a sélectionné un groupe spécifique
        if ($isAdmin && $forcedKey && $forcedKey !== 'global' && $forcedKey !== 'retd') {
            $activeGroup = \App\Models\Group::where('key', $forcedKey)->first();
            $groupName = $activeGroup ? $activeGroup->name : null;
        } 
        // Sinon (Utilisateur normal), on regarde sa base de données
        elseif (auth()->user()->franchise_id) {
            $userGroup = \App\Models\Group::find(auth()->user()->franchise_id);
            $groupName = $userGroup ? $userGroup->name : null;
        }
    }
@endphp
<main class="max-w-6xl w-full mx-auto p-6 flex-1">
    ...
    @else
        {{-- SI ON EST DANS L'ONGLET R&D GLOBAL (Mode Dossiers) --}}
        @if($tab === 'all' && in_array('retd', session('keycloak_groups', [])) && empty($selectedFolder))
                ...
                @if($showAllDocs)
                    <a href="{{ route('home', ['tab' => 'all', 'folder' => 'ALL_DOCS']) }}" 
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-indigo-600 dark:text-indigo-400 mt-0.5">{{ collect($documents)->flatten(1)->count() }} document(s)</p>
                        </div>
                    </a>
                @endif
        ...
        {{-- AFFICHAGE STANDARD DES CARTES DOCUMENTS (Tous les autres onglets) --}}
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                @foreach($documents as $doc)
                    <div class="relative bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-5 shadow-sm hover:shadow-md transition flex flex-col justify-between h-48">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-gray-900 dark:text-white truncate transition-colors duration-200 {{ $tab === 'all' ? 'pr-20' : '' }}">{{ $doc->title }}</h3>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (searchInput && searchForm) {
            searchInput.addEventListener('input', function() {
                ...
                if (searchInput.value.endsWith(' ')) return;
                timeout = setTimeout(function() { searchForm.submit(); }, 500); 
            });
